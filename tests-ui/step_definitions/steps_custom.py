    ...
    with soft_assertions():
        for url in urls:
            ...
            assert_that(response.status_code,
                        description=f'URL {url} load status code is {response.status_code}').is_less_than_or_equal_to(
                307)
...
@when(parsers.re("I search keywords '(?P<text>.*)' in field '(?P<locator_path1>.*)' then I click '(?P<locator_path2>.*)' and compare results count in '(?P<locator_path3>.*)'"), converters=dict(locator_path=str))
def compare_search_result(selenium, selenium_generics: SeleniumGenerics, text, locators: Locators, locator_path1, locator_path2, locator_path3):
    xpath1 = locators.parse_and_get(locator_path1)
    xpath2 = locators.parse_and_get(locator_path2)
    xpath3 = locators.parse_and_get(locator_path3)
    result = selenium_generics.search_compare_result_count(xpath1, xpath2, xpath3, text)
    assert_that(result).is_equal_to(True)


### Custom code for scenario outline
#@when(parsers.parse("I enter text <(?P<text>.*)> to field '(?P<locator_path>.*)'"))
@when("I enter text <text>")
def set_element_text(selenium, text):
    logging.Logger.info("the text is "+text)
    # print(locator_path)
    # element = selenium.find_element(By.XPATH, locators.parse_and_get(locator_path))
    # element.send_keys(text)

# @when(parsers.re("I scroll '(?P<text>.*)' in the page "))
# def compare_search_result(selenium, selenium_generics: SeleniumGenerics, text, locators: Locators, locator_path1, locator_path2):
#     xpath = locators.parse_and_get(locator_path1)
#     xpath1 = locators.parse_and_get(locator_path2)
#     result = selenium_generics.search_compare_result_count(xpath, xpath1, text)
#     assert_that(result).is_equal_to(True)
